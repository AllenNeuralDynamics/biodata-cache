"""Platform fiber photometry cache table."""
...
@registry.register_table(registry.NAMES["fib"])
def platform_fib(force_update: bool = False) -> pd.DataFrame:
    ...
    if df.empty or force_update:
        ...
        registry.BACKEND.write(registry.NAMES["fib"], df)

    return df


def platform_fib_columns() -> list[Column]:
    """Return platform_fib cache table column definitions."""
    return [
        ...
        Column(
            name="patch_cord", description="Patch cord name (e.g. 'Patch Cord 0'), from Patch cord config device_name"
        ),
