...
@registry.register_table("metadata_upgrade")
def metadata_upgrade(force_update: bool = False) -> pd.DataFrame:
    ...
    """
    df = registry.BACKEND.read("metadata_upgrade")

    if df.empty or force_update:
        setup_logging()
        logging.info(
            CacheLogMessage(
                backend=registry.BACKEND.__class__.__name__, table="metadata_upgrade", message="Updating upgrade status"
            ).to_json()
        )
        ...
        basics_df = registry.TABLE_REGISTRY[registry.NAMES["basics"]]()
        ...
        if not failed.empty:
            ...
            v1_df = (
                pd.DataFrame(v1_records)
                if v1_records
                else pd.DataFrame(columns=["_id", "name", "project_name", "data_level"])
            )
        ...
        registry.BACKEND.write("metadata_upgrade", final_df)
        return final_df
...
def metadata_upgrade_columns() -> list[Column]:
    """Return metadata upgrade cache table column definitions."""
    return [
