"""Unique project names cache table."""

import logging

import pandas as pd
from aind_data_access_api.document_db import MetadataDbClient

import biodata_cache.registry as registry
from biodata_cache.models import Column
from biodata_cache.utils import (
    CacheLogMessage,
    setup_logging,
)


@registry.register_table(registry.NAMES["upn"])
def unique_project_names(force_update: bool = False) -> list[str]:
    """Fetch unique project names from metadata database.

    Returns cached results if available, fetches from database if cache is empty
    or force_update is True.

    Args:
        force_update: If True, bypass cache and fetch fresh data from database.

    Returns:
        List of unique project names.

    """
    df = registry.BACKEND.read(registry.NAMES["upn"])

    if df.empty and not force_update:
        raise ValueError("Cache is empty. Use force_update=True to fetch data from database.")

    if df.empty or force_update:
        setup_logging()
        logging.info(
            CacheLogMessage(
                backend=registry.BACKEND.__class__.__name__, table=registry.NAMES["upn"], message="Updating cache"
            ).to_json()
        )
        client = MetadataDbClient(
            host=registry.API_GATEWAY_HOST,
            version="v2",
        )
        unique_project_names = client.aggregate_docdb_records(
            pipeline=[
                {"$group": {"_id": "$data_description.project_name"}},
                {"$project": {"project_name": "$_id", "_id": 0}},
            ]
        )
        df = pd.DataFrame(unique_project_names)
        registry.BACKEND.write(registry.NAMES["upn"], df)

    return df["project_name"].dropna().tolist()


def unique_project_names_columns() -> list[Column]:
    """Return unique project names cache table column definitions."""
    return [
        Column(name="project_name", description="Project name"),
    ]
