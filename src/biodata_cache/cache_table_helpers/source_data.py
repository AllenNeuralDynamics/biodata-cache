"""Source data cache table - unified derived asset table."""
...
@registry.register_table(registry.NAMES["d2r"])
def source_data(force_update: bool = False) -> pd.DataFrame:
    ...
    """
    df = registry.BACKEND.read(registry.NAMES["d2r"])
    ...
    if df.empty or force_update:
        ...
        registry.BACKEND.write(registry.NAMES["d2r"], df)

    return df


def source_data_columns() -> list[Column]:
    """Return source data cache table column definitions."""
    return [
