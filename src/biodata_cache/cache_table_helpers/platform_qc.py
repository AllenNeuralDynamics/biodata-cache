"""Platform QC cache table: per-platform tag-level quality control status tables."""
...
@registry.register_table("platform_qc")
...
def _build(platform: str) -> pd.DataFrame:
    ...
    meta = platform_df[
        ["name", "acquisition_start_time", "instrument_id_normalized", "experimenters_normalized"]
    ].rename(columns={"name": "asset_name", "acquisition_start_time": "timestamp"})
...
def platform_qc_columns() -> list[Column]:
    """Return platform_qc cache table column definitions."""
    return [
        ...
        Column(
            ...
            description="Normalized, deduplicated, sorted list of experimenter display names",
        ),
    ]
