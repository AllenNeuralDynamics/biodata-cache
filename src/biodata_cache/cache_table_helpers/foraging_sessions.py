"""Foraging sessions cache table: one row per behavior session from df_sessions.pkl."""
...
from biodata_cache.utils import CacheLogMessage, normalize_name, setup_logging
...
@registry.register_table(registry.NAMES["foraging"])
def foraging_sessions(force_update: bool = False) -> pd.DataFrame:
    ...
    """
    df = registry.BACKEND.read(registry.NAMES["foraging"])
    ...
    if df.empty or force_update:
        ...
        logging.info(
            CacheLogMessage(
                ...
                table=registry.NAMES["foraging"],
                message="Updating cache from S3 pkl",
            ).to_json()
        )

        df = _fetch_foraging_sessions()
        registry.BACKEND.write(registry.NAMES["foraging"], df)

    return df


def foraging_sessions_columns() -> list[Column]:
    """Return foraging_sessions cache table column definitions."""
    return [
