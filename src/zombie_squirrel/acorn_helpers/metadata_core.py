"""Metadata core files presence acorn."""
...
@acorns.register_acorn(acorns.NAMES["core"])
def metadata_core(force_update: bool = False) -> pd.DataFrame:
    ...
    """
    df = acorns.TREE.scurry(acorns.NAMES["core"])
    ...
    if df.empty or force_update:
        ...
        logging.info(
            SquirrelMessage(
                tree=acorns.TREE.__class__.__name__, acorn=acorns.NAMES["core"], message="Updating cache"
            ).to_json()
        )
        ...
        acorns.TREE.hide(acorns.NAMES["core"], df)

    return df


def metadata_core_columns() -> list[Column]:
    """Return metadata core acorn column definitions."""
    return [
