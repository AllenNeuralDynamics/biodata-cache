...
@acorns.register_acorn(acorns.NAMES["curriculum"])
def behavior_curriculum(force_update: bool = False) -> pd.DataFrame:
    ...
    """
    df = acorns.TREE.scurry(acorns.NAMES["curriculum"])
    ...
    incomplete = set(df[df["curriculum_name"].notna() & df["stage_node_id"].isna()]["asset_name"].dropna()) if not df.empty else set()
    ...
    for i, (_, row) in enumerate(new_assets.iterrows()):
        ...
        if not location.startswith(f"s3://{AIND_OPEN_DATA_BUCKET}/"):
            new_rows.append({"asset_name": asset_name, "curriculum_name": None, "stage_name": None, "stage_node_id": None})
        else:
        ...
        if (i + 1) % CHECKPOINT_INTERVAL == 0:
            ...
            acorns.TREE.hide(acorns.NAMES["curriculum"], checkpoint_df)
            logging.info(
                SquirrelMessage(
                    tree=acorns.TREE.__class__.__name__,
                    acorn=acorns.NAMES["curriculum"],
    ...
    acorns.TREE.hide(acorns.NAMES["curriculum"], df)

    return df


def behavior_curriculum_columns() -> list[Column]:
    """Return behavior_curriculum acorn column definitions."""
    return [
        ...
        Column(name="stage_node_id", description="Integer node ID of the current stage in the curriculum graph; use for ordering stages"),
    ]
