"""Unit tests for unique_genotypes acorn."""
...
from zombie_squirrel.acorn_helpers.unique_genotypes import (
    unique_genotypes,
    unique_genotypes_columns,
)


@patch("zombie_squirrel.acorn_helpers.unique_genotypes.acorns.TREE")
def test_unique_genotypes_empty_cache_raises(mock_tree):
...
@patch("zombie_squirrel.acorn_helpers.unique_genotypes.acorns.TREE")
def test_unique_genotypes_cache_hit(mock_tree):
    mock_tree.scurry.return_value = pd.DataFrame({"genotype": ["Ai32", "Ai148"]})
    result = unique_genotypes(force_update=False)
    assert result == ["Ai32", "Ai148"]


@patch("zombie_squirrel.acorn_helpers.unique_genotypes.MetadataDbClient")
@patch("zombie_squirrel.acorn_helpers.unique_genotypes.acorns.TREE")
def test_unique_genotypes_force_update(mock_tree, mock_client_class):
    mock_tree.scurry.return_value = pd.DataFrame()
...
@patch("zombie_squirrel.acorn_helpers.unique_genotypes.MetadataDbClient")
@patch("zombie_squirrel.acorn_helpers.unique_genotypes.acorns.TREE")
def test_unique_genotypes_excludes_null(mock_tree, mock_client_class):
    mock_tree.scurry.return_value = pd.DataFrame()
    mock_client = MagicMock()
    mock_client_class.return_value = mock_client
    mock_client.aggregate_docdb_records.return_value = [
...
@patch("zombie_squirrel.acorn_helpers.unique_genotypes.MetadataDbClient")
@patch("zombie_squirrel.acorn_helpers.unique_genotypes.acorns.TREE")
def test_unique_genotypes_pipeline_filters_nulls(mock_tree, mock_client_class):
    mock_tree.scurry.return_value = pd.DataFrame()
    mock_client = MagicMock()
    mock_client_class.return_value = mock_client
