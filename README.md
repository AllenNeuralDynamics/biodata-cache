...
![Interrogate](https://img.shields.io/badge/interrogate-40.7%25-red)
...
| `platform_qc` | Tag-level QC statuses aggregated per platform, one row per asset/tag combination | `s3://allen-data-views/data-asset-cache/zs_platform_qc/` | platform | True (by `platform`) | `asset_name`, `subject_id`, `instrument_id`, `experimenter`, `tag`, `status`, `timestamp` |
